"""Modal jobs for the standalone scGPT benchmark robustness analyses.
...
app = modal.App("scgpt-standalone-robustness-v3")
...
_REVISION_ANALYSIS_LOCAL = _HERE / "revision_analysis_v3.py"
_AGGREGATE_ANALYSIS_LOCAL = _HERE / "aggregate_v3.py"
...
benchmark_image = (
    ...
    .add_local_file(str(_REVISION_ANALYSIS_LOCAL), remote_path="/root/revision_analysis_v3.py")
    .add_local_file(str(_AGGREGATE_ANALYSIS_LOCAL), remote_path="/root/aggregate_v3.py")
)
...
HUMAN_GROUP_COLUMNS = {
    "TNBC_Breast_Cancer": "donor_id",
    "Indonesia_PBMC": "donor_id",
    "Brain_Atlas": "donor_id",
    "Multi_Tissue_TME": "donor_id",
    "Human_Pancreas": "donor_id",
}
...
def audit_all_grouping() -> str:
    """Audit donor, sample and batch identifiers before grouped evaluation."""
    ...
    for dataset_name in DATASETS:
        adata = ad.read_h5ad(f"/vol/data/{dataset_name}.h5ad", backed="r")
        record = {
        ...
        for column in grouping_candidates:
            ...
            valid = values.notna() & ~values.astype(str).isin(("", "nan", "None"))
            as_string = values.loc[valid].astype(str)
            per_class = pd.crosstab(labels.loc[valid], as_string)
            record["available_grouping_columns"][column] = {
                "n_nonmissing_cells": int(valid.sum()),
                "n_missing_cells": int((~valid).sum()),
                "n_unique": int(as_string.nunique()),
                "min_cells_per_group": int(as_string.value_counts().min()) if len(as_string) else 0,
                "max_cells_per_group": int(as_string.value_counts().max()) if len(as_string) else 0,
                "classes_per_group_min": int((per_class > 0).sum(axis=0).min()) if per_class.shape[1] else 0,
                "classes_per_group_max": int((per_class > 0).sum(axis=0).max()) if per_class.shape[1] else 0,
                "groups_per_class": {
                    str(label): int((per_class.loc[label] > 0).sum())
                    for label in per_class.index
                },
            }
        payload[dataset_name] = record
        adata.file.close()
    destination = Path("/vol/results/robustness_v3/grouping_audit.json")
...
def audit_dataset_citations() -> str:
    """Record the source title and citation embedded in every public H5AD."""
    import anndata as ad
    ...
    for dataset_name in DATASETS:
        ...
        payload[dataset_name] = {
            ...
            "citation": str(adata.uns.get("citation", "")),
            "organism": str(adata.uns.get("organism", "")),
            "schema_version": str(adata.uns.get("schema_version", "")),
        }
        adata.file.close()
    destination = Path("/vol/results/robustness_v3/dataset_citations.json")
    destination.parent.mkdir(parents=True, exist_ok=True)
    destination.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    volume.commit()
    return json.dumps(payload, indent=2)
...
def run_grouped_human_task_gpu(task: str) -> str:
    """Run one dataset/fold task encoded as ``dataset::fold``."""
    sys.path.insert(0, "/root")
    from revision_analysis_v3 import run_grouped_human_fold_v3

    dataset_name, fold_text = task.rsplit("::", 1)
    if dataset_name not in HUMAN_GROUP_COLUMNS:
        raise ValueError(f"Unknown human dataset: {dataset_name}")
    fold = int(fold_text)
    if fold not in range(5):
        raise ValueError(f"Unknown fold: {fold}")
    payload = run_grouped_human_fold_v3(
        dataset_name=dataset_name,
        group_column=HUMAN_GROUP_COLUMNS[dataset_name],
        fold=fold,
        data_path=f"/vol/data/{dataset_name}.h5ad",
        model_dir="/vol/models/scGPT_human",
        output_root="/vol/results/grouped_human_v3",
        n_splits=5,
    )
    volume.commit()
    return json.dumps(payload, indent=2)
...
def run_strict_scarcity_task_gpu(fold: int) -> str:
    """Run one labelled-only PBMC scarcity fold."""
    sys.path.insert(0, "/root")
    from revision_analysis_v3 import run_strict_scarcity_fold_v3

    if fold not in range(5):
        raise ValueError(f"Unknown fold: {fold}")
    payload = run_strict_scarcity_fold_v3(
        fold=fold,
        data_path="/vol/data/Indonesia_PBMC.h5ad",
        model_dir="/vol/models/scGPT_human",
        output_root="/vol/results/strict_scarcity_v3",
    )
    volume.commit()
    return json.dumps(payload, indent=2)
...
def run_pig_stress_gpu() -> str:
    """Run the separately reported, independently tuned pig stress test."""
    sys.path.insert(0, "/root")
    from revision_analysis_v3 import run_pig_cross_species_stress_v3

    payload = run_pig_cross_species_stress_v3(
        data_path="/vol/data/Pig_Pancreas.h5ad",
        cache_root="/vol/results/atlas_benchmark_final_v2",
        output_root="/vol/results/pig_cross_species_v3",
    )
    volume.commit()
    return json.dumps(payload, indent=2)
...
def aggregate_v3_cpu() -> str:
    """Aggregate all final human, scarcity and cross-species outputs."""
    sys.path.insert(0, "/root")
    from aggregate_v3 import aggregate_v3_results

    payload = aggregate_v3_results(
        grouped_root="/vol/results/grouped_human_v3",
        scarcity_root="/vol/results/strict_scarcity_v3",
        pig_root="/vol/results/pig_cross_species_v3",
        output_root="/vol/results/robustness_v3_aggregate",
        n_bootstrap=5000,
    )
    volume.commit()
    return json.dumps(payload, indent=2)
...
def main(stage: str = "metadata", dataset: str = "Brain_Atlas", fold: int = 0):
    if stage == "audit-grouping":
        print(audit_all_grouping.remote())
    elif stage == "audit-citations":
        print(audit_dataset_citations.remote())
    elif stage == "grouped-one":
        if dataset not in HUMAN_GROUP_COLUMNS:
            raise ValueError(f"Unknown human dataset: {dataset}")
        if fold not in range(5):
            raise ValueError(f"Unknown fold: {fold}")
        print(run_grouped_human_task_gpu.remote(f"{dataset}::{fold}"))
    elif stage == "grouped-dataset":
        if dataset not in HUMAN_GROUP_COLUMNS:
            raise ValueError(f"Unknown human dataset: {dataset}")
        tasks = [f"{dataset}::{fold_index}" for fold_index in range(5)]
        for result in run_grouped_human_task_gpu.map(tasks, order_outputs=True):
            print(result)
    elif stage == "grouped-all":
        tasks = [
            f"{dataset_name}::{fold_index}"
            for dataset_name in HUMAN_GROUP_COLUMNS
            for fold_index in range(5)
        ]
        for result in run_grouped_human_task_gpu.map(tasks, order_outputs=True):
            print(result)
    elif stage == "strict-scarcity-one":
        if fold not in range(5):
            raise ValueError(f"Unknown fold: {fold}")
        print(run_strict_scarcity_task_gpu.remote(fold))
    elif stage == "strict-scarcity-all":
        for result in run_strict_scarcity_task_gpu.map(range(5), order_outputs=True):
            print(result)
    elif stage == "pig-stress":
        print(run_pig_stress_gpu.remote())
    elif stage == "aggregate-v3":
        print(aggregate_v3_cpu.remote())
    elif stage == "metadata":
