...
def _v3_candidate_grid(classifier: str) -> list[dict]:
    """Small, prespecified grids used independently within each representation."""
    if classifier == "logreg":
        return [
            {"C": float(c_value), "scale": bool(scale)}
            for scale in (False, True)
            for c_value in (1e-3, 1e-2, 1e-1, 1.0, 10.0, 100.0, 1000.0)
        ]
    ...
    if classifier == "xgboost":
        return [
            ...
            {"max_depth": 9, "learning_rate": 0.05, "n_estimators": 400},
        ]
    raise ValueError(classifier)


def _v3_fit_classifier(classifier: str, params: dict, X_train, y_train, seed: int):
    """Fit one classifier candidate with representation-appropriate scaling."""
    import scipy.sparse as sp
    ...
    if classifier == "logreg":
        ...
        if params.get("scale", False):
            ...
            model = Pipeline((("scale", scaler), ("classifier", estimator)))
        else:
            model = estimator
...
def _v3_tune_classifier(
    ...
    validation_position,
    seed: int,
    cap_per_partition: int = 50000,
):
    """Select hyperparameters on an inner split and return the full trace."""
    import numpy as np
    from sklearn.metrics import f1_score

    fit_position = _stratified_cap(
        fit_position,
        y,
        cap=cap_per_partition,
        seed=seed + 11,
    )
    validation_position = _stratified_cap(
        validation_position,
        y,
        cap=cap_per_partition,
        seed=seed + 29,
    )
    X_fit, X_validation = _classifier_views(
        classifier,
        X[fit_position],
        X[validation_position],
    )
    trace = []
    best_params = None
    best_score = -np.inf
    for candidate_index, params in enumerate(_v3_candidate_grid(classifier)):
        candidate_started = time.time()
        fitted = _v3_fit_classifier(
            classifier,
            params,
            X_fit,
            y[fit_position],
        ...
        trace.append(
            {
                ...
                "n_tuning_fit_cells": int(len(fit_position)),
                "n_tuning_validation_cells": int(len(validation_position)),
    ...
    if best_params is None:
        raise RuntimeError(f"No hyperparameter candidate completed for {classifier}")
    return best_params, best_score, trace


def _v3_inner_group_positions(y_train, groups_train, seed: int):
    """Return disjoint tuning-fit, validation and calibration positions."""
    import numpy as np
    ...
    if n_splits < 3:
        raise ValueError("At least three training groups are required for nested tuning")
    splitter = StratifiedGroupKFold(
        n_splits=n_splits,
        shuffle=True,
        random_state=seed,
    )
    assignment = np.full(len(y_train), -1, dtype=np.int16)
    for inner_fold, (_, held_position) in enumerate(
        splitter.split(np.zeros(len(y_train), dtype=np.uint8), y_train, groups_train)
    ):
        assignment[held_position] = inner_fold
    if np.any(assignment < 0):
        raise AssertionError("Incomplete inner group-fold assignment")
    classes = set(np.unique(y_train))
    for validation_fold in range(n_splits):
        for calibration_fold in range(n_splits):
            if calibration_fold == validation_fold:
                continue
            validation = np.flatnonzero(assignment == validation_fold)
            calibration = np.flatnonzero(assignment == calibration_fold)
            fit = np.flatnonzero(
                (assignment != validation_fold) & (assignment != calibration_fold)
            )
            if all(set(np.unique(y_train[position])) == classes for position in (fit, validation, calibration)):
                if set(groups_train[fit]).intersection(groups_train[validation]):
                    raise AssertionError("Group leakage into tuning validation split")
                if set(groups_train[fit]).intersection(groups_train[calibration]):
                    raise AssertionError("Group leakage into calibration split")
                if set(groups_train[validation]).intersection(groups_train[calibration]):
                    raise AssertionError("Group leakage between validation and calibration")
                return fit, validation, calibration, assignment
    raise RuntimeError("Could not construct class-complete inner group partitions")


def _v3_complete_outer_group_folds(y, groups, n_splits: int, base_seed: int):
    """Find one deterministic SGKF assignment with every class in every fold."""
    import numpy as np
    from sklearn.model_selection import StratifiedGroupKFold
    ...
    for offset in range(100):
        ...
        folds = list(
            splitter.split(np.zeros(len(y), dtype=np.uint8), y, groups)
        )
        if all(
            set(np.unique(y[train])) == expected
            and set(np.unique(y[test])) == expected
            for train, test in folds
        ):
            return folds, random_state
    raise RuntimeError("No class-complete outer StratifiedGroupKFold assignment found")


def _v3_logits(model, classifier: str, X):
    """Return multiclass logits suitable for scalar temperature scaling."""
    import numpy as np
...
def _v3_tune_labeled_logreg(X, y, seed: int):
    """Tune Logistic Regression using only the labelled scarcity subset."""
    import numpy as np
    from sklearn.metrics import f1_score
    ...
    best_params = None
    best_score = -np.inf
    for candidate_index, params in enumerate(_v3_candidate_grid("logreg")):
        fold_scores = []
        for train_position, validation_position in splitter.split(
            np.zeros(len(y), dtype=np.uint8), y
        ):
            fitted = _v3_fit_classifier(
                "logreg",
                params,
            ...
            prediction = fitted.predict(X[validation_position])
            fold_scores.append(
                float(
                    f1_score(
                        y[validation_position],
...
def run_grouped_human_fold_v3(
    ...
    """Run one nested, group-held-out human-atlas fold with tuned classifiers."""
    import anndata as ad
    import numpy as np
    import pandas as pd
    import scanpy as sc
    ...
    if dataset_name == "Pig_Pancreas":
        raise ValueError("Pig pancreas is reserved for the cross-species stress test")
    if str(SCGPT_SOURCE_DIR) not in sys.path:
        sys.path.insert(0, str(SCGPT_SOURCE_DIR))
    import scgpt

    seed = 20261111 + 100 * ATLAS_NAMES.index(dataset_name) + fold
    configure_reproducibility(seed)
    destination = Path(output_root) / dataset_name / f"fold_{fold}"
    destination.mkdir(parents=True, exist_ok=True)
    marker = destination / "complete.json"
    if marker.exists():
        return json.loads(marker.read_text(encoding="utf-8"))
    ...
    started = time.time()
    print(f"[{dataset_name} fold {fold}] loading data", flush=True)
    adata = ad.read_h5ad(data_path)
    if adata.raw is None:
        raise ValueError(f"{dataset_name} lacks raw counts")
    if group_column not in adata.obs:
        raise KeyError(f"{dataset_name} lacks grouping column {group_column}")
    label_series = adata.obs["cell_type"].astype("string")
    group_series = adata.obs[group_column].astype("string")
    valid = label_series.notna() & group_series.notna()
    counts = label_series.loc[valid].value_counts()
    retained_labels = counts[counts > 5].index
    retained_index = np.flatnonzero(
        (valid & label_series.isin(retained_labels)).to_numpy()
    )
    categorical = pd.Categorical(label_series.iloc[retained_index])
    y = categorical.codes.astype(np.int64)
    class_names = [str(value) for value in categorical.categories]
    groups = group_series.iloc[retained_index].astype(str).to_numpy()
    if len(np.unique(groups)) < n_splits:
        raise ValueError(
            f"{dataset_name} has {len(np.unique(groups))} groups for {n_splits} folds"
        )
    folds, outer_random_state = _v3_complete_outer_group_folds(
        y,
        groups,
        n_splits=n_splits,
        base_seed=20261111 + ATLAS_NAMES.index(dataset_name),
    )
    train_position, test_position = folds[fold]
    train_index = retained_index[train_position]
    test_index = retained_index[test_position]
    if set(groups[train_position]).intersection(groups[test_position]):
        raise AssertionError("Outer group leakage detected")
    expected_classes = set(range(len(class_names)))
    if set(np.unique(y[train_position])) != expected_classes:
        raise AssertionError("Outer training partition is missing a class")
    if set(np.unique(y[test_position])) != expected_classes:
        raise AssertionError("Outer test partition is missing a class")
    active_index = np.concatenate((train_index, test_index))
    y_active = np.concatenate((y[train_position], y[test_position]))
    groups_active = np.concatenate((groups[train_position], groups[test_position]))
    n_train = len(train_index)

    print(f"[{dataset_name} fold {fold}] selecting genes within outer training groups", flush=True)
    train_counts = _raw_slice_to_adata(adata, train_index)
    sc.pp.filter_genes(train_counts, min_counts=3)
    sc.pp.highly_variable_genes(
        train_counts,
    ...
    selected_feature_names = matched["mapped_feature_name"].astype(str).tolist()
    if len(selected_var_names) < 100:
        raise ValueError(f"Only {len(selected_var_names)} genes matched the scGPT vocabulary")
    (destination / "selected_genes.json").write_text(
        json.dumps(
            {
                "dataset": dataset_name,
    ...
    print(f"[{dataset_name} fold {fold}] extracting official scGPT embeddings", flush=True)
    raw_selected = _raw_slice_to_adata(adata, active_index, selected_var_names)
    raw_selected.var["feature_name"] = selected_feature_names
    if sp.issparse(raw_selected.X):
        raw_selected.X = raw_selected.X.tocsr(copy=True)
        raw_selected.X.eliminate_zeros()
        nonzero_input = np.diff(raw_selected.X.indptr) > 0
    else:
        nonzero_input = np.count_nonzero(np.asarray(raw_selected.X), axis=1) > 0
    n_zero_train = int((~nonzero_input[:n_train]).sum())
    n_zero_test = int((~nonzero_input[n_train:]).sum())
    if n_zero_train or n_zero_test:
        ...
        active_index = active_index[nonzero_input]
        y_active = y_active[nonzero_input]
        groups_active = groups_active[nonzero_input]
        n_train = int(nonzero_input[:n_train].sum())
        train_index = active_index[:n_train]
        test_index = active_index[n_train:]
    if set(np.unique(y_active[:n_train])) != expected_classes:
        raise AssertionError("Shared-input QC removed an outer-training class")
    if set(np.unique(y_active[n_train:])) != expected_classes:
        raise AssertionError("Shared-input QC removed an outer-test class")
    embedded = scgpt.tasks.embed_data(
        raw_selected,
        model_dir=model_dir,
        gene_col="feature_name",
    ...
    embeddings = np.asarray(embedded.X, dtype=np.float32)
    if embeddings.shape != (len(active_index), 512):
        raise ValueError(f"Unexpected embedding shape {embeddings.shape}")
    if not np.isfinite(embeddings).all():
        raise ValueError("Non-finite scGPT embeddings")
    embedding_norms = np.linalg.norm(embeddings, axis=1)
    if not np.allclose(embedding_norms, 1.0, atol=1e-5):
        raise ValueError("Official scGPT embeddings were expected to be unit normalized")
    np.save(destination / "active_indices.npy", active_index)
    del embedded
    torch.cuda.empty_cache()

    print(f"[{dataset_name} fold {fold}] constructing expression and SVD controls", flush=True)
    sc.pp.normalize_total(raw_selected, target_sum=1e4)
    sc.pp.log1p(raw_selected)
    expression = raw_selected.X
    expression = (
    ...
    del raw_selected, adata
    svd_fit_position = _stratified_cap(
        np.arange(n_train, dtype=np.int64),
        y_active[:n_train],
        cap=100000,
        seed=seed + 401,
    )
    n_components = min(512, expression.shape[1] - 1, len(svd_fit_position) - 1)
    if n_components != 512:
        raise ValueError(f"Could not fit the prespecified 512-component SVD: {n_components}")
    svd = TruncatedSVD(
        n_components=512,
        algorithm="randomized",
        n_iter=5,
        random_state=seed + 409,
    )
    svd.fit(expression[svd_fit_position])
    svd_expression = svd.transform(expression).astype(np.float32)
    (destination / "svd_audit.json").write_text(
        json.dumps(
            {
                "n_components": 512,
                "n_training_cells_used_to_fit": int(len(svd_fit_position)),
                "explained_variance_ratio_sum": float(svd.explained_variance_ratio_.sum()),
                "fit_restricted_to_outer_training_cells": True,
            },
            indent=2,
        ),
        encoding="utf-8",
    ...
    representations = {
        ...
        "svd512_expression": svd_expression,
    }
    y_train = y_active[:n_train]
    y_test = y_active[n_train:]
    groups_train = groups_active[:n_train]
    fit_position, validation_position, calibration_position, inner_assignment = (
        _v3_inner_group_positions(y_train, groups_train, seed=seed + 503)
    )
    np.save(destination / "inner_group_assignment.npy", inner_assignment)

    result_rows = []
    temperature_rows = []
    tuning_payload = []
    for representation_name, X in representations.items():
        for classifier in ("logreg", "random_forest", "xgboost"):
            model_name = f"{representation_name} plus {classifier}"
            print(f"[{dataset_name} fold {fold}] tuning {model_name}", flush=True)
            best_params, best_score, trace = _v3_tune_classifier(
                classifier,
                X,
                y_train,
                fit_position,
                validation_position,
                seed=seed + 1000 * list(representations).index(representation_name) + 100 * ("logreg", "random_forest", "xgboost").index(classifier),
            )
            tuning_payload.append(
                {
                    "dataset": dataset_name,
                    "fold": fold,
            ...
            np.savez_compressed(
                ...
                test_index=test_index.astype(np.int64),
                group_id=groups_active[n_train:].astype(str),
            ...
            result_rows.append(
                {
                    ...
                    "classifier": classifier,
                    "model": model_name,
                    "selected_parameters": json.dumps(best_params, sort_keys=True),
                    "inner_validation_macro_f1": best_score,
                    "n_train_cells": n_train,
                    "n_test_cells": len(y_test),
                    "n_train_groups": int(len(np.unique(groups_active[:n_train]))),
                    "n_test_groups": int(len(np.unique(groups_active[n_train:]))),
                    "n_matched_hvgs": len(selected_var_names),
                    "fit_and_predict_seconds": time.time() - final_started,
                    **metrics,
                }
            )
            del fitted, probability, X_train_view, X_test_view

            if classifier in ("logreg", "xgboost"):
                noncalibration_position = np.concatenate((fit_position, validation_position))
                X_noncalibration, X_calibration = _classifier_views(
                    classifier,
                    X[noncalibration_position],
                    X[calibration_position],
                )
                _, X_temperature_test = _classifier_views(
                    classifier,
                    X[noncalibration_position[:1]],
                    X[n_train:],
                )
                calibration_model = _v3_fit_classifier(
                    classifier,
                    best_params,
                    X_noncalibration,
                    y_train[noncalibration_position],
                    seed=seed + 8003,
                )
                calibration_logits = _v3_logits(
                    calibration_model,
                    classifier,
                    X_calibration,
                )
                ...
                for calibrated_flag, calibration_probability in (
                    (False, uncalibrated),
                    (True, calibrated),
                ):
                    temperature_rows.append(
                        {
                            ...
                            "selected_parameters": json.dumps(best_params, sort_keys=True),
                            "n_classifier_fit_cells": int(len(noncalibration_position)),
                            "n_calibration_cells": int(len(calibration_position)),
                            "n_test_cells": int(len(y_test)),
                            "n_classifier_fit_groups": int(len(np.unique(groups_train[noncalibration_position]))),
                            "n_calibration_groups": int(len(np.unique(groups_train[calibration_position]))),
                            "n_test_groups": int(len(np.unique(groups_active[n_train:]))),
                            **calculate_metrics(y_test, calibration_probability),
                        }
                    )
                del (
                    calibration_model,
    ...
    pd.DataFrame(temperature_rows).to_csv(
        destination / "temperature_scaling_metrics.csv",
        index=False,
    )
    (destination / "tuning.json").write_text(
    ...
    payload = {
        ...
        "fold": fold,
        "group_column": group_column,
        "outer_split_random_state": outer_random_state,
        "n_unique_groups_total": int(len(np.unique(groups))),
        "n_train_groups": int(len(np.unique(groups_active[:n_train]))),
        "n_test_groups": int(len(np.unique(groups_active[n_train:]))),
        ...
        "n_zero_input_train_excluded": n_zero_train,
        "n_zero_input_test_excluded": n_zero_test,
        "n_pipelines": len(result_rows),
        "n_temperature_rows": len(temperature_rows),
        "total_seconds": time.time() - started,
    }
    marker.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    print(f"[{dataset_name} fold {fold}] complete in {payload['total_seconds'] / 60:.1f} min", flush=True)
    return payload


def run_strict_scarcity_fold_v3(
    fold: int,
    data_path: str,
    model_dir: str,
    output_root: str,
    labels_per_class=(10, 50, 100, 250, 500),
    scarcity_seeds=(20261201, 20261202),
) -> dict:
    """PBMC sensitivity in which feature selection uses labelled cells only."""
    import anndata as ad
    import numpy as np
    import pandas as pd
    import scanpy as sc
    ...
    adata = ad.read_h5ad(data_path)
    labels = pd.Categorical(adata.obs["cell_type"].astype("string"))
    y = labels.codes.astype(np.int64)
    ...
    folds, outer_random_state = _v3_complete_outer_group_folds(
        ...
        groups,
        n_splits=5,
        base_seed=20261111 + ATLAS_NAMES.index("Indonesia_PBMC"),
    )
    train_index, test_index = folds[fold]
    if set(groups[train_index]).intersection(groups[test_index]):
        raise AssertionError("Donor leakage in strict scarcity outer fold")
    vocab_dict = json.loads((Path(model_dir) / "vocab.json").read_text(encoding="utf-8"))
    result_rows = []
    tuning_payload = []
    started = time.time()
    for labels_target in labels_per_class:
        for scarcity_seed in scarcity_seeds:
            rng = np.random.default_rng(scarcity_seed + 1000 * fold + labels_target)
            selected_parts = []
            for class_index in range(len(class_names)):
                candidates = train_index[y[train_index] == class_index]
                if len(candidates) < labels_target:
                    raise ValueError(
                        f"Class {class_names[class_index]} has {len(candidates)} cells, "
                        f"below the requested {labels_target}"
                    )
                selected_parts.append(
                    rng.choice(candidates, size=labels_target, replace=False)
                )
            labelled_index = np.concatenate(selected_parts)
            rng.shuffle(labelled_index)
            y_labelled = y[labelled_index]

            print(
                f"[strict scarcity fold {fold}] {labels_target}/class seed {scarcity_seed}: "
                "labelled-only HVG selection",
                flush=True,
            )
            labelled_counts = _raw_slice_to_adata(adata, labelled_index)
            sc.pp.filter_genes(labelled_counts, min_counts=3)
            sc.pp.highly_variable_genes(
                labelled_counts,
                n_top_genes=min(1200, labelled_counts.n_vars),
                flavor="seurat_v3",
                subset=False,
            )
            ranked = labelled_counts.var.loc[
                labelled_counts.var["highly_variable"]
            ].copy()
            ranked = ranked.sort_values("highly_variable_rank", kind="mergesort")
            if "feature_name" not in ranked:
                ranked["feature_name"] = ranked.index.astype(str)
            ...
            if len(selected_var_names) < 100:
                raise ValueError(
                    f"Only {len(selected_var_names)} vocabulary-matched genes in strict scarcity"
                )
            del labelled_counts, ranked, matched

            active_index = np.concatenate((labelled_index, test_index))
            n_labelled = len(labelled_index)
            raw_selected = _raw_slice_to_adata(adata, active_index, selected_var_names)
            raw_selected.var["feature_name"] = selected_feature_names
            if sp.issparse(raw_selected.X):
                raw_selected.X = raw_selected.X.tocsr(copy=True)
                ...
                nonzero_input = np.count_nonzero(np.asarray(raw_selected.X), axis=1) > 0
            if not nonzero_input[:n_labelled].all():
                raise AssertionError("A labelled scarcity cell had zero shared-gene input")
            raw_selected = raw_selected[nonzero_input].copy()
            active_index = active_index[nonzero_input]
            n_labelled_after = int(nonzero_input[:n_labelled].sum())
            if n_labelled_after != n_labelled:
                raise AssertionError("Strict scarcity labelled count changed after shared-input QC")
            y_test = y[active_index[n_labelled:]]
            ...
            embeddings = np.asarray(embedded.X, dtype=np.float32)
            if embeddings.shape[1] != 512 or not np.isfinite(embeddings).all():
                raise ValueError("Invalid strict-scarcity scGPT embeddings")
            del embedded
            sc.pp.normalize_total(raw_selected, target_sum=1e4)
            sc.pp.log1p(raw_selected)
            expression = raw_selected.X
            ...
            for representation_name, X in (
                ...
                best_params, validation_score, trace = _v3_tune_labeled_logreg(
                    X[:n_labelled],
                    y_labelled,
                    seed=scarcity_seed + fold,
                )
                fitted = _v3_fit_classifier(
                    "logreg",
                    best_params,
                    X[:n_labelled],
                    y_labelled,
                    seed=scarcity_seed + 5000 + fold,
                )
                probability = fitted.predict_proba(X[n_labelled:]).astype(np.float32)
                result_rows.append(
                    {
                        "fold": fold,
                        "labels_per_class": int(labels_target),
                        "scarcity_seed": int(scarcity_seed),
                        "representation": representation_name,
                        "classifier": "logreg",
                        "selected_parameters": json.dumps(best_params, sort_keys=True),
                        "inner_cv_macro_f1": validation_score,
                        "n_labelled_cells": int(n_labelled),
                        "n_test_cells": int(len(y_test)),
                        "n_test_donors": int(len(np.unique(groups[active_index[n_labelled:]]))),
                        "n_matched_hvgs": int(len(selected_var_names)),
                        "n_zero_test_excluded": int((~nonzero_input[n_labelled:]).sum()),
                        "feature_selection_scope": "labelled cells only",
                ...
                del fitted, probability
            del embeddings, expression
            torch.cuda.empty_cache()

    pd.DataFrame(result_rows).to_csv(destination / "strict_scarcity_metrics.csv", index=False)
    (destination / "strict_scarcity_tuning.json").write_text(
        json.dumps(tuning_payload, indent=2),
        encoding="utf-8",
    )
    payload = {
        ...
        "n_test_donors": int(len(np.unique(groups[test_index]))),
        "total_seconds": time.time() - started,
    }
    marker.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    return payload
...
def run_pig_cross_species_stress_v3(
    data_path: str,
    cache_root: str,
    output_root: str,
) -> dict:
    """Retune the cached human-checkpoint pig analysis as a separate stress test."""
    import anndata as ad
    import numpy as np
    import pandas as pd
    import scanpy as sc
    ...
    seed = 20261301
    configure_reproducibility(seed)
    source = Path(cache_root) / "Pig_Pancreas"
    ...
    class_names = [str(value) for value in source_complete["class_names"]]

    adata = ad.read_h5ad(data_path)
    labels = pd.Categorical(adata.obs["cell_type"].astype("string"), categories=class_names)
    y_active = labels.codes.astype(np.int64)[active_index]
    ...
    sample_column = "sample" if "sample" in adata.obs else "sample_id"
    sample_ids = adata.obs[sample_column].astype(str).to_numpy()[active_index]
    ...
    all_train = np.arange(n_train, dtype=np.int64)
    fit_position, validation_position = train_test_split(
        all_train,
        test_size=0.20,
        random_state=seed + 17,
        stratify=y_active[:n_train],
    )
    representations = {
        "scgpt_embeddings": embeddings,
        "matched_expression": expression,
    ...
    tuning_payload = []
    for representation_name, X in representations.items():
        for classifier_index, classifier in enumerate(("logreg", "random_forest", "xgboost")):
            best_params, best_score, trace = _v3_tune_classifier(
                classifier,
                X,
                y_active[:n_train],
                fit_position,
                validation_position,
                seed=seed + 1000 * list(representations).index(representation_name) + 100 * classifier_index,
            )
            X_train, X_test = _classifier_views(classifier, X[:n_train], X[n_train:])
            fitted = _v3_fit_classifier(
                classifier,
                best_params,
                X_train,
            ...
            probability = fitted.predict_proba(X_test).astype(np.float32)
            if not np.array_equal(fitted.classes_, np.arange(len(class_names))):
                raise AssertionError("Unexpected pig stress-test class order")
            model_name = f"{representation_name} plus {classifier}"
            np.savez_compressed(
                destination / f"predictions__{_model_slug(model_name)}.npz",
                dataset=np.asarray("Pig_Pancreas"),
                test_index=active_index[n_train:].astype(np.int64),
                sample_id=sample_ids[n_train:].astype(str),
                y_true=y_active[n_train:].astype(np.int16),
                y_prob=probability,
                class_names=np.asarray(class_names, dtype=str),
                representation=np.asarray(representation_name),
            ...
            rows.append(
                {
                    "dataset": "Pig_Pancreas",
                    "analysis_role": "cross-species stress test",
                    "representation": representation_name,
                    "classifier": classifier,
                    "model": model_name,
                    "selected_parameters": json.dumps(best_params, sort_keys=True),
                    "inner_validation_macro_f1": best_score,
                    "n_train_cells": n_train,
                    "n_test_cells": int(len(y_active) - n_train),
                    "n_samples_total": int(len(np.unique(sample_ids))),
                    "n_matched_hvgs": int(len(genes["var_names"])),
                    **calculate_metrics(y_active[n_train:], probability),
                }
            )
            tuning_payload.append(
                {
            ...
            del fitted, probability, X_train, X_test
    pd.DataFrame(rows).to_csv(destination / "metrics.csv", index=False)
    (destination / "tuning.json").write_text(
        json.dumps(tuning_payload, indent=2), encoding="utf-8"
    )
    payload = {
        "dataset": "Pig_Pancreas",
        "analysis_role": "cross-species stress test",
        "checkpoint_species": "human",
        "feature_mapping": "exact symbol, then uppercase symbol; no orthology mapping",
        "n_samples_total": int(len(np.unique(sample_ids))),
        "n_train_cells": n_train,
        "n_test_cells": int(len(y_active) - n_train),
        "n_pipelines": len(rows),
        "svd512_explained_variance_ratio_sum": svd_variance,
        "total_seconds": time.time() - started,
    }
    marker.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    return payload
